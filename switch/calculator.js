...
const SWITCH_1 = 37, SWITCH_2 = 38;
//Switch states
//Because javascript is evil, switchStates = {SWITCH_1 : false}
//sets "SWITCH_1" as the actual key.
var switchStates = {};
switchStates[SWITCH_1] = false;
switchStates[SWITCH_2] = false;

//APerforms an action based on this switch.
const SWITCH_DIR_UPDOWN = 0, SWITCH_DIR_LEFTRIGHT = 1;
var switchDir = 0;
function activateSwitch(theSwitch){
    switch(theSwitch){
        case SWITCH_1:
            //This switch controls up/down, then left/right.
            if(switchDir == SWITCH_DIR_UPDOWN){
                //on == down, off == up.
                if(switchStates[theSwitch]){
                    selectDown();
                }
                else
                {
            ...
            {
                //This is leftright. on == right, off == left.
                if(switchStates[theSwitch]){
                    selectNext();
                }
                else 
            ...
            break;
        case SWITCH_2:
            //This switch controls selection.
            if(switchDir == SWITCH_DIR_UPDOWN)
            {
                //This switch either switches direction, then goes into leftright, or simply activates.
                if(switchStates[theSwitch]){
                    //Go into leftright.
                    switchDir = SWITCH_DIR_LEFTRIGHT;
                    $("#statusarea") .text("Moving let/right.");
                }
                else
                {
                    clickSelectedItem();
                    $("#statusarea").text("Clicked current item.");
                }
            }
            else
            {
                //on == Activate this switch, off == nothing. In both, go back to updown.
                switchDir = SWITCH_DIR_UPDOWN;
                if(switchStates[theSwitch])
                {
                    clickSelectedItem();
                    $("#statusarea").text("Activated item.");
                }
                else 
                {
                    $("#statusarea").text("moving up and down.");
                }
            }
    }
    
}
//This allows us to set intervals for whether a switch has a toggled state on.
function toggleSwitch(theSwitch){
    let curStatus = switchStates[theSwitch];
    if(curStatus === false){
        switchStates[theSwitch] = 1+setTimeout(()=>{
            activateSwitch(theSwitch);
            switchStates[theSwitch] = false;
        }, 900);
    }
    else{
        clearInterval(curStatus-1);
        switchStates[theSwitch] = false;
        activateSwitch(theSwitch);
    }
}
